...
try:
    from simple_agent.core.rich_output import print_info
    RICH_AVAILABLE = True
except ImportError:
...
class StartDaemonCommand(CommandHandler):
    ...
    def execute(self, args: List[str], context: Dict[str, Any]) -> CommandResult:
        try:
            from simple_agent.services.daemon import get_daemon
            daemon = get_daemon()
...
class InstallServiceCommand(CommandHandler):
    ...
    def execute(self, args: List[str], context: Dict[str, Any]) -> CommandResult:
        try:
            from simple_agent.services.daemon import generate_systemd_service, generate_launchd_plist
            import platform
