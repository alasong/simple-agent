...
2. 条件分支：根据结果选择下一个 Agent
3. 自动生成：根据任务描述自动创建 Workflow
4. 上下文共享：所有 Agent 共享工作流上下文
5. 多类型结果：支持字符串、文件、结构化数据
6. 调试输出：支持将各步骤结果保存到文件

注意：并行执行支持已移至 core.workflow_parallel 模块
"""
...
# ==================== 并行执行支持 (已移至 core.workflow_parallel) ====================
# 从新模块导入并行执行相关类和函数，保持向后兼容
from .workflow_parallel import ParallelWorkflow, ParallelStep, create_parallel_workflow
from .workflow_types import ParallelExecutionResult
